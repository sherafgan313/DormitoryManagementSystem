-- Dormitory Management System — Universal Database Dump
-- Compatible with: MySQL 5.7+, MySQL 8.0+, MariaDB 10.3+
-- Tested environments:
--   WampServer (Windows)   — MySQL 8.0.31
--   XAMPP 8.2.4-0 (Linux)  — MariaDB 10.x
--   AMPPS 4.4 (Mac)        — MySQL 5.7.x / 8.x
--
-- USAGE:
--   mysql -u root -p < Documentation/dorm_management_all.sql
-- OR from phpMyAdmin: import this file directly.
--
-- NOTE: server.js will skip seeding because data already exists in this dump.
...
-- ─── DROP ALL TABLES ─────────────────────────────────────────────────────────
-- FK checks are disabled above so drop order does not matter.
...
-- Passwords:
--   admin@dms.com    → admin123
--   student@dms.com, juan@dms.com, ana@dms.com,
--   pedro@dms.com, rosa@dms.com  → student123
...
INSERT INTO `dorm_applications` VALUES
...
-- payment_id 1 and 2 were deleted; AUTO_INCREMENT starts at 3 to preserve original IDs.
...
-- ─── NOTIFICATIONS ───────────────────────────────────────────────────────────
-- Changed from MyISAM to InnoDB for FK support and cross-platform consistency.
-- Multiple NULL reference_id values per user are allowed (MySQL/MariaDB treat
...
INSERT INTO `termination_requests` VALUES
...
ALTER TABLE `termination_requests`
  ADD CONSTRAINT `termreq_contract_fk` FOREIGN KEY (`contract_id`) REFERENCES `contracts` (`contract_id`),
  ADD CONSTRAINT `termreq_user_fk`     FOREIGN KEY (`user_id`)     REFERENCES `users` (`user_id`);
